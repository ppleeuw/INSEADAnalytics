...
The high positive correlations make sense, as these are mostly related futures: soybeans and soybean oil, wheat and corn. Some of them are more ambiguous however, such as the pound and Australian dollar or TSE500 and TSE Toronto. These can be explained by effects on global developments. The negative correlctions are all very low and cannot logically be explained.
...
**Your Answers here:**
<br>1. The cumulative percentage of explained variance is above 50 at component 6, so we need at least 6 factors
<br>2. The eigenvalue of the first component is 14.04 which means that this component explains 14.04 variables. The variables which count most in coponent 1 are 5 yr T Notes US and 10yr T-Notes, both with 0.93
<br>3. These are all treasury notes/bonds, I would call this component 'Bonds'
<br>4. Please see below
<br>5. There are less valid components during bearmarkets, which means assets are less correlated during bearyears than during bullyears
...
```{r Fig5, echo=FALSE, comment=NA, results='asis', message=FALSE, fig.align='center', fig=TRUE}
df           <- cbind(as.data.frame(eigenvalues), c(1:length(eigenvalues)), rep(1, length(eigenvalues)))
colnames(df) <- c("eigenvalues", "components", "abline")
Line         <- gvisLineChart(as.data.frame(df), xvar="components", yvar=c("eigenvalues","abline"), options=list(title='Scree plot', legend="right", width=900, height=600, hAxis="{title:'Number of Components', titleTextStyle:{color:'black'}}", vAxes="[{title:'Eigenvalues'}]",  series="[{color:'green',pointSize:3, targetAxisIndex: 0}]"))
print(Line, 'chart')
...
Here are the first 20 rotated principal components:
<br>
...
<br>
**Your Answer here:**
<br> The average performance of our fund is 6.25 % over the years 2001-2015 while that of the Societe Generale fund is 5.12 %.
<br>
#### Questions
...
**Your Answers here:**
<br> 1. The code calculates moving averages and indicates whether it is a positive or negative moving average and by how much. It then distributes 1 dollar every day distributed among these numbers with a buy or sell and proportion according to the moving average signs and value.
<br>2. The command line seq(10,250,by=20) gives a sequence in beginning with 10 and ending with 250 (this formula looks back to 250 days of volatility) in steps of 20. This is the moving average scope used. Lets see what happen if we take a shorter one with shorter steps (from 10 to 150 with steps of 10).
...
<br>
<br>This strategy gives an average return of 6.38 %, which is higher than the previous one of 6.25 %.
<br>
<br>
...
1. How does your trend following strategy compare with the “mix-and-match” strategy you built above? Which one do you prefer and why?
2. Why/when does it work, and why/when does it not work? Why?
3. What can you tell about the correlation of the staregy you developed with S&P?

<br>

**Your Answers here:**
<br> 1. & 2. The trend following strategy performs better over the long term, but the mix and match strategy has less risk it seems. Since I am young, I prefer the trend following strategy since it is still relatively low in risk.
<br>3. The first one seems to be more correlated to the S&P as it follow the trend index more.

...
**Your Answers here:**
<br>There is an interactive brokers package which provides native R access directly to the investment software API I have been using for years 
<br>I am keen to find out what could be done with this (e.g. give commands to trade when certain data rules have been met)
...
**Your Answers here:**
<br>Question 1.: In dataSet1.R there is a variable called mytickers that takes bot SPY and AAPL data
<br>
<br>Question 2: The cumulative returns of the Apple index during this period is `r round(100*sum(StockReturns[,2]),1)`%.
<br> The average daily returns of the Apple index during this period is `r round(100*mean(StockReturns[,2]),3)`%;
<br> The standard deviation of the daily returns of the Apple index during this period is `r round(100*sd(StockReturns[,2]),3)`%;
<br>
<br>
<br>Question 3: yes, by adjusting the dataset1.R file, adding "YHOO" to mytickers = c("SPY", "AAPL", "YHOO"), save it and rerun the previous process, with the number 3 instead of 2 in the commands. The results are cumulative 288.802, mean 0.072, SD 3.02
...
to get the number of rows, we use nrow(transposedData): `r nrow(transposedData)`
<br>to get the number of columns, we use ncol(transposedData): `r ncol(transposedData)`
<br>
<br>
<br>Question 2: apply(transposedData,2,mean) calculates the mean for the columns of the transposedData matrix. which is an average of the return for AAPL and SPY (50-50 split)
<br>Therefore to plot this portfolio one needs to plot apply(transposedData,2,mean)
...
**Your Answers here:**
<br>Question 1: change to startDate = "2001-01-01" (was already there in the file though, so no need to change)
<br>Question 2: Yes, this can be done by adding companies to mytickers, e.g. c("SPY", "AAPL", "TSLA") and change the startdate to the one wanted. Then we rerun it again
...
**Your Answers here:**
<br>Question 1: this gives the number of instances in which myData is not equal to StockReturns[1:20] = `r sum(myData != StockReturns[1:20,])`
<br>Although both datasets have the same numbers, they are not exactly equal due to rounding differences
<br>Question 2: both tables will be added to each other, giving us the doubles for the first 20 rows. After the first 20 rows, mydata will be added again to the second 20 numbers of StockReturns.
